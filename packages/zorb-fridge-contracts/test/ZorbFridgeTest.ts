...
import { ZorbFridge, ZorbFridge__factory, ZorbNFT, ZorbNFT__factory } from "../typechain";
...
describe("ZorbFridge", () => {
  ...
  beforeEach(async () => {
    const { ZorbFridge, ZorbNFT } = await deployments.fixture(["ZorbNFT", "ZorbFridge"]);
  ...
  it("allows owner of zorb to freeze and unfreeze", async () => {
    ...
    expect(await fridge.ownerOf(1)).to.be.equal(ethers.constants.AddressZero);
		...
		console.log(await fridge.zorb())
		console.log(zorb.address)
    ...
    expect(await fridge.connect(signer2).unfreeze(1)).to.emit(fridge, "Unfrozen");
  });
  ...
  it("allows owner of zorb to freeze and unfreeze multiple times", async () => {
    ...
    expect(await fridge.freeze(1)).to.emit(fridge, "Frozen");
    await expect(fridge.connect(signer2).unfreeze(1)).to.be.revertedWith("not owner");
	})
});
